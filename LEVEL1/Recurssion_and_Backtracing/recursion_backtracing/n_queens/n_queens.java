
import java.io.*;
import java.util.*;
public class n_queens 
{

    public static void main(String[] args) throws Exception {
        Scanner sc = new Scanner(System.in);
        int n = sc.nextInt();
        int [][] chess = new int[n][n];
        printNQueens(chess, "", 0);
    }

    public static void printNQueens(int[][] chess, String qsf, int row) {   // row = levels
        if(row == chess.length)
        {
            System.out.println(qsf);
            return;
        }
        for(int col = 0 ; col<chess.length ; col++)       // col = options
        {
            if(isitsafeplace(chess , row , col) == true)
            {
                chess[row][col] = 1;
                printNQueens(chess, qsf + row + "-" + col + ",", row + 1);
                chess[row][col] = 0;
            }
        }
    }

    // public static boolean isitsafeplace(int[][] chess , int row , int col)
    // {
    //     for(int i = row - 1 , j = col-1 ; i>=0 ; i++)   //vertical
    //     {
    //         if(chess[row][col] == 1)
    //         {
    //             return false;
    //         }
    //     }

    //     for(int i = row - 1 , j = col-1 ; i>=0 && j>=0 ; i-- , j--)   //  left diagnal
    //     {
    //         if(chess[row][col] == 1)
    //         {
    //             return false;
    //         }

    //     }

    //     for(int i = row-1 , j=col+1 ; i>=0 && j <chess.length ; i-- , j++)    // right diagnal
    //     {
    //         if(chess[row][col] == 1)
    //         {
    //             return false;
    //         }
    //     }   

    //     return true;
    // }

    public static boolean isitsafeplace (int[][] chess,int row, int col) {
    for (int i = row - 1, j = col - 1;i >= 0 && j >= 0; i--, j--)      // left diagnal
    {
      if (chess[i][j] == 1) 
      {
        return false;
      }
    }

    for (int i = row - 1, j = col; i >= 0; i--) {            // vertical
      if (chess[i][j] == 1) 
      {
        return false;
      }
    }

    for (int i = row - 1, j = col + 1; i >= 0 && j < chess.length; i--, j++)     // right diagnal 
    {
      if (chess[i][j] == 1)
      {
        return false;
      }
    }

    for (int i = row, j = col - 1; j >= 0; j--)          // horizontal
    {
      if (chess[i][j] == 1) 
      {
        return false;
      }
    }

    return true;
  }
}
